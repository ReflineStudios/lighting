...
static Transform sModelTransform
{
    ...
    { 0.0f, -32.0f, 0.0f },   // rotation
    { 1.0f, 1.0f, 1.0f }    // scale
};

struct Camera
{
    Vec3 location = { 0.0, 0.0f, 0.0f };
...
static std::vector<SubMeshData> sSubMeshes;
...
void Init()
{
    ...
    // vertex buffer, index buffer, textures

    std::string mesh = "dbd";
    std::string texExt = ".png";
    uint32_t texNum = 8;
    ...
    std::string meshPath = "meshes/" + mesh + ".obj";
...
# if 0
    ...
    for (uint32_t i = 0; i < texNum; i++)
    {
        ...
        std::string filename = "textures/" + mesh + "/" + std::to_string(i) + texExt;
        int x, y, channels;
        void* img = stbi_load(filename.c_str(), &x, &y, &channels, 4);
        ...
        d3dcheck(gDevice->CreateTexture2D(&texDesc, &texData, &texture));

        stbi_image_free(img);

        ID3D11ShaderResourceView* textureView;
        d3dcheck(gDevice->CreateShaderResourceView(texture, nullptr, &textureView));

        sSubMeshes[i].texture = textureView;
    }
    ...
    D3D11_SUBRESOURCE_DATA texData = {};
    texData.pSysMem = &texData0;
    texData.SysMemPitch = 1 * 4;
    ...
    for (auto& s : sSubMeshes)
        s.texture = textureView;
    ...
    uint32_t offsets[] = { 0 };
    gContext->IASetVertexBuffers(0, 1, &vertexBuffer, strides, offsets);
...
int APIENTRY WinMain(HINSTANCE hInst, HINSTANCE hInstPrev, PSTR cmdline, int cmdshow)
{
    ...
    gWindow = CreateWindowExA(0, "boh", "3D Renderer!", WS_CAPTION | WS_MAXIMIZEBOX | WS_MINIMIZEBOX | WS_SIZEBOX | WS_SYSMENU,
